...
import { getStatusInvoicesInSap } from '../services/sap.service.js'
...
console.log('datas',data)
  ...
  const getColumnSearchProps = dataIndex => ({
    ...
    render: text =>
     ...
     const getEnlaceInvoices = async(record) =>{
      console.log(record?.DocNum)
        const DocNum=record?.DocNum
        try {
          const response = await getStatusInvoicesInSap(DocNum)
          console.log('respuestaFac',response)
        } catch (error) {
            SetAlert({
          ok: true,
          tipo: "info",
  ...
  const columns = [
    ...
    >
      {/*<ButtonCustom disabled={'false'} onClick={()=>getEnlaceInvoices(record)} text={'Factura'}></ButtonCustom>*/}
      <Modal onClick={getEnlaceInvoices(record.DocNum)} formulario='VerFacturaReserva' URL_PDF={record?.U_V3_FCE_Enlace} title={record?.U_V3_FCE_Enlace ? 'VER FACTURA' : 'SIN FACTURA' }/>
    </div>
      )
    },
